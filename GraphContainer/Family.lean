...
For a `q`-set `S`:
* if `S` is independent, its container is `S ∪ activeAfter G q S`;
* otherwise its container is just `S`.
-/
...
noncomputable def container (q : ℕ) (S : Finset V) : Finset V :=
  S ∪ if G.IsIndepSet S then activeAfter G q S else ∅
...
private theorem card_container_le {β : ℝ} {R q : ℕ} (h : Hypotheses G β R q)
    {S : Finset V} (hSq : #S = q) :
    #(container G q S) ≤ R + q := by
  by_cases hS : G.IsIndepSet S
  · rw [container, if_pos hS]
    calc
      #(S ∪ activeAfter G q S)
          ≤ #S + #(activeAfter G q S) := Finset.card_union_le _ _
      _ ≤ q + R := Nat.add_le_add hSq.le
        (card_activeAfter_le G h hS hSq.ge)
      _ = R + q := Nat.add_comm _ _
  · rw [container, if_neg hS, Finset.union_empty, hSq]
...
private theorem independent_subset_some_container {q : ℕ}
  ...
  · refine ⟨fingerprint G q I, ?_, ?_⟩
    · exact mem_fingerprints.mpr (card_fingerprint_of_le_card G hI hqI)
    · rw [container, if_pos (fingerprint_isIndepSet G hI q)]
...
theorem graph_container_theorem {β : ℝ} {R q : ℕ}
  ...
  classical
  exact ⟨containerFamily G q, card_containerFamily_le G q,
...
the active set obtained by replaying that fingerprint form a certificate. -/
theorem fingerprint_certificate {β : ℝ} {R q : ℕ}
    (h : Hypotheses G β R q)
    {I : Finset V} (hI : G.IsIndepSet I) (hqI : q ≤ #I) :
    IsCertificate q R I (fingerprint G q I)
      (activeAfter G q (fingerprint G q I)) := by
  have hrun : run G (fingerprint G q I) q = run G I q :=
